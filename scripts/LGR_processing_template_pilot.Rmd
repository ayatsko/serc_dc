...
d1query$CH4_d_L <- 
  ...
  (d1query$CH4/1000000) * d1query$Volume * 1000 
#CO2
d1query$CO2_d_L <- 
  # parts CO2 per million parts air * volume of air in chamber (m3) * 1000 L per m3
  (d1query$CO2/1000000) * d1query$Volume * 1000 
...
d1query$CH4_d_umol <- 
  ...
  ((1*d1query$CH4_d_L)/(0.08206*(d1query$AmbT_C+273)))*10^6
# CO2
d1query$CO2_d_mmol <- 
  # (atm pressure * L CO2) / (R in L*atm/?K*mol * ?K temp) 
  ((1*d1query$CO2_d_L)/(0.08206*(d1query$AmbT_C+273)))*10^3

# for day 2 
d2query$CH4_d_L <- (d2query$CH4/1000000) * d2query$Volume * 1000 
d2query$CO2_d_L <- (d2query$CO2/1000000) * d2query$Volume * 1000 
d2query$CH4_d_umol <- ((1*d2query$CH4_d_L)/(0.08206*(d2query$AmbT_C+273)))*10^6
d2query$CO2_d_mmol <- ((1*d2query$CO2_d_L)/(0.08206*(d2query$AmbT_C+273)))*10^3
```

now visualize fluxes for both co2 and ch4 in terms of moles GHG / day: 
```{r graph seperate samples}
# day 1 co2 (plot 1) and ch4 (plot 2)
plot1 <- ggplot(d1query, aes(fDOY, CO2_d_mmol)) + 
  geom_point() + 
  facet_wrap( ~ PIECETAG, ncol = 3, scales = "free") + 
  stat_smooth(method = "lm") +
  theme_classic()
plot2 <- ggplot(d1query, aes(fDOY, CH4_d_umol)) + 
  geom_point() + 
  facet_wrap( ~ PIECETAG, ncol = 3, scales = "free") + 
  stat_smooth(method = "lm") +
  theme_classic()
...
# day 2 co2 (plot 3) and ch4 (plot 4)
plot3 <- ggplot(d2query, aes(fDOY, CO2_d_mmol)) + 
  geom_point() + 
  facet_wrap( ~ PIECETAG, ncol = 3, scales = "free") + 
  stat_smooth(method = "lm") +
  theme_classic()
plot4 <- ggplot(d2query, aes(fDOY, CH4_d_umol)) + 
  geom_point() + 
  facet_wrap( ~ PIECETAG, ncol = 3, scales = "free") + 
  stat_smooth(method = "lm") +
  theme_classic()
...
# samp_152539A
samp_152539A <- lm(CH4_d_umol ~ fDOY, samp_150650A) 
summary(samp_152539A)
