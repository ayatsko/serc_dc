...
now we should have all our data in a more meaningful, manageable, and consistent form (when it comes to variables especially). here they are:

```{r list polished data frames}
View(dw2014)
View(dw2017)
View(dead_2019)
View(living_2019) # note on nomenclature - dead within the living 2019 survey - so techincally all these pieces are actually dead and not living 
```
...
abbey: TASKS
  ...
  * look into cache = TRUE - this is a thing that skips over time-consuming chunks that have byproducts already existing in the R environment
...
important.cols <- c("PIECETAG", "STEMTAG", "QUADNAME", "QX.x", "QY.x", "NOTES.2021")
...
#### 9. INITIAL PLOTS FOR DW DISTRIBUTION ----
...
# create plots by year that show species distribution 

sp2014 <- ggplot(data = subset(data, !is.na(SPCODE)), aes(x=SPCODE)) + 
  geom_bar(position="stack", stat="count")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```

#### 10. TRANSITION MATRIX BRAINSTORM ----

what are the probabilities of transition between the different states (1-3 decay classes) across three time points (2014, 2017, 2021)?
```{r}
# convert 2017 and 2021 data to 3 tiered decay class scale (I think this is the way that Jess broke it down in the past)
# 1 = 1
# 2 = 2,3
# 3 = 4,5
...
```{r}
# make a class 6 for 2017 'consumed' (in BDS) and 2021 'not found' (in notes) - in excel

# new.df <- data.frame(DC.2017 = c(1, 2, 1, 5, 4, 3, 2), DC.2021 = c(2, 2, 3, 6, 6, 5, 2))
...
# percent of going from column to row (2017 is column, 2021 is row)
