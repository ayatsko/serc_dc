...
# merge stemtag and piece for new identifier 
# dw2014 
...
# living_2019 
sum(is.na(living_2019$PIECE.2021)) # 18 NAs in the piece vector 
...
# what species are these shared stemtags (2014-2017)? this could be useful to look at later
...
ok now for actual merge based on the col_common variable - this is all wrong i think (as it previously was)
```{r merge}
...
# I think this is problematic because it doesnt actually keep much of the info (those outside of col_common are omitted??)

# try to do a leftjoin of just 2014 and 2017
# left_join(dw2014, dw2017, by = "STEMTAG") -> dw_merged
# left_join(dw_merged, dead_2019, by = "STEMTAG") -> dw_merged
# left_join(dw_merged, living_2019, by = "STEMTAG") -> dw_merged
# # I think a problem with this method is that when the join happens and there are A and B pieces, everything is assigned the decay class that is merged. therefore it might be best for STEMTAG to be merged with PIECE so that this does not start to misrepresent other 'pieces' 

# ALTERNATIVELY... 
# THIS ISNT WORKING :"\
# dw2014 %>%
#   full_join(dw2017, by = intersect(colnames(dw2014), colnames(dw2017))) %>%
#   group_by(STEMTAG)-> test2
# 
# test2 %>% 
# full_join(dead_2019, by = intersect(colnames(test2), colnames(dead_2019))) %>%
#   group_by(STEMTAG) -> test3

# just look at stemtag data and decay classes - see what holds here 
# subset out everything else
# names(dw_merged)
# dw_merged_dc <- select(dw_merged, c("STEMTAG", "DC.2014", "DC.2021.x", "DC.2017", "DC.2021.y", "DC.2021.x.x",  "DC.2021.y.y","SPCODE.x", "SPCODE.y", "SPCODE.x.x", "SPCODE.y.y"))
# 
# 
# # what stemtags have values for both 2014 and 2017?
# dw_merged_dc %>% drop_na(DC.2014, DC.2017) -> test 
# sum(is.na(dw_merged_dc$DC.2014)) #1
# sum(is.na(dw_merged_dc$DC.2017)) #1133
# this makes sense because 'test' is narrowed down to 470 obs. 

# now I guess the trouble is that the dc 2021 variable is spread out like all heck! maybe this could be fixed above in a more thrifty manner? or I could attempt it here from the other end of things... 
...
# VARIABLE.x, VARIABLE.y, VARIABLE.x.x, VARIABLE.y.y --> these four variables all represent the same 'baseline' variable, but since they are coming from 4 different original locations (dw2014, dw2017, dead_2019, living_2019), they are parsed seperately. the goal is to bring them all together into one single VARIABLE 
...
q.col <- grep("HEIGHT.2021", names(dw_total))
...
# important.cols <- "PIECETAG", "STEMTAG", "QUADNAME"
# new.df <- dw_total[, important.cols]

# STEP 3
# select variables to correct and bind to new dataframe
name.list <- c("BDS.2021", "INTEG.2021", "DC.2021", "GC.2021", "TOPDIAM.2021", "DBH.2021", "LENGTH.2021", "SPCODE", "HEIGHT.2021")

# STEP 4 
for (n in 1:length(name.list)) {
  new.df <- expand.dataframe(new.df, dw_total, name.list[n])
}
head(new.df)
...
expand.dataframe <-  function(new.df, df, NAME){
  ...
  assign(new.col, paste(NAME, "2021", sep = "."))
