...
#### 5. CREATE DECAY CLASS TRANSITION SIMULATIONS ----
...
N[, 1] <- matrix(c(300, 100, 0, 0, 0), ncol = 1)
...
```{r}
# trying to add pulses of DC = 1 each year 
...
# flip matrix for new input values. should only be a row of values (inputs)
N <- t(N)

# add 10 to [1,1] element of N
N[1,1] <- N[1,1] + 10
...
N <- as.vector(N)
t1 <- rowSums(N)
# these values in the first column of N are the output numbers of each decay class. 
# they need to be transposed to the first row as inputs for the second year
...
# sum across rows for new inputs 
N <- (rowSums(N))
as.vector(N)
